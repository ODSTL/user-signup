...
def user_signup_complete():
    ...
    err_char_count = "Password must be between 3 and 20 characters."
    err_no_spaces = "Password must not contain spaces."
    ...
    if empty_val(email):
        ...
        if not char_length(email):
            ...
            email_error = "Email address must contain only one @ symbol."
            password = ''
            password_validate = ''
            password_error = err_reenter_pw
            password_validate_error = err_reenter_pw
